...
import attackUtils as utils
from numba import jit
import torch
import network

def calc_degree(adjs):
    degrees = []
    for adj in adjs:
        adj = np.array(adj)
...
class NasBench101SearchSpaceAttack:
    def __init__(self, src_batch:dgl.DGLGraph, src_batch_acc:torch.Tensor, u, verbose=False ):
        self.src_batch = src_batch
        self.src_batch_acc = src_batch_acc
        self.u = u

    def start_attacking_by_modify_edge(self):
        unit = 7
        cpy = self.src_batch.ndata['attr']
        self.src_batch.ndata['attr'] = 1 - self.src_batch.ndata['attr']
        print(cpy==self.src_batch.ndata['attr'])
        return self.src_batch



class SearchSpaceAttackOnSingleOne:
    """
    ...
    def __init__(self, adj, X_obs, z_obs, W1, W2, u, verbose=False):
        ...
        # Node attributes
        self.X_obs = self.X_obs.copy().tolil()
    ...
    def compute_logits(self):
        ...
        #看一下尺寸
        print(f"adj_preprocessed:{self.adj_preprocessed.shape},X_obs_shape:{self.X_obs.shape},W:{self.W.shape}")
        return self.adj_preprocessed.dot(self.adj_preprocessed).dot(self.X_obs.dot(self.W.T))[self.u].toarray()[0]
